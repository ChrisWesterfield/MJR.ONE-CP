...
class XhguiRuns implements iXHProfRuns, ContainerAwareInterface
{
    ...
    public function save_run($xhprof_data, $type, $run_id = null) {
        $pmu = isset($xhprof_data['main()']['pmu']) ? $xhprof_data['main()']['pmu'] : 0;
        $wt  = isset($xhprof_data['main()']['wt'])  ? $xhprof_data['main()']['wt']  : 0;
        $cpu = isset($xhprof_data['main()']['cpu']) ? $xhprof_data['main()']['cpu'] : 0;
        ...
        $em = $doctrine->getManager($this->container->getParameter('mjr_library_profiler.entity_manager'));
        $entityClass  = $this->container->getParameter('mjr_library_profiler.entity_class');
