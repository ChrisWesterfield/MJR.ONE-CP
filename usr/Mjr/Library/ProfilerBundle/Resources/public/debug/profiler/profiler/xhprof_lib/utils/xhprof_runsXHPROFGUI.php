...
/**
 * XHProfRuns_DefaultXHPROFGUI is the default implementation of the
 ...
 */
class XHProfRuns_DefaultXHPROFGUI implements iXHProfRuns {
...
*/
  ...
  */
  public function getRuns($stats)
  {
      ...
      {
        $query = "SELECT {$stats['select']} FROM system_profiler ";
      }else
      {
        $query = "SELECT * FROM system_profiler ";
      }
  ...
  public function getDistinct($data)
  {
	...
	$query = "SELECT DISTINCT({$sql['column']}) FROM system_profiler";
  ...
  */
  public function get_run($run_id, $type, &$run_desc) 
  {
    ...
    $query = "SELECT * FROM system_profiler WHERE id = '$run_id'";
    $resultSet = $this->db->query($query);
    ...
    foreach($data as $id=>$value)
    {
        ...
        {
            $data[$id] = stream_get_contents($value);
        }
    }
    ...
    $contents = gzuncompress($data['perfdata']);
    ...
    switch($GLOBALS['_xhprof']['serializer'])
    {
        ...
        break;
        case 'igbinary':
            $contents = igbinary_unserialize($contents);
        break;
    }
  ...
  */
  public function getRunComparativeData($url, $c_url)
  {
      ...
      $query = 'SELECT count(id), avg(wt), min(wt), max(wt),  avg(cpu), min(cpu), max(cpu), avg(pmu), min(pmu), max(pmu) FROM system_profiler WHERE url = \''.$url.'\'';
      $rs = $this->db->query($query);
      ...
      //Runs same c_url
      //  count, avg/min/max for wt, cpu, pmu
      $query = 'SELECT count(id), avg(wt), min(wt), max(wt),  avg(cpu), min(cpu), max(cpu), avg(pmu), min(pmu), max(pmu) FROM system_profiler WHERE c_url = \''.$c_url.'\'';
      $rs = $this->db->query($query);
      $row = $this->db->getNextAssoc($rs);
      $row['url'] = $c_url;
  ...
  protected function calculatePercentile($details)
  {
                  ...
                  $query = "SELECT {$details['column']} as value FROM system_profiler WHERE {$details['type']} = '{$details['url']}' ORDER BY {$details['column']} DESC LIMIT 1 OFFSET $limit";
                  $rs = $this->db->query($query);
                  $row = $this->db->getNextAssoc($rs);
                  return $row['value'];
  }
