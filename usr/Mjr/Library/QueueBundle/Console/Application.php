...
namespace Mjr\Library\QueueBundle\Console;
...
class Application extends BaseApplication
{
    ...
    public function __construct(KernelInterface $kernel)
    {
        ...
        $this->getDefinition()->addOption(new InputOption('--job-id', null, InputOption::VALUE_REQUIRED, 'The ID of the Job.'));

        $kernel->boot();
        if ($kernel->getContainer()->getParameter('mjr_library_queue.statistics')) {
            $this->insertStatStmt = "INSERT INTO system_job_statistics (job_id, characteristic, createdAt, charValue) VALUES (:jobId, :name, :createdAt, :value)";
    ...
    {
        if ( ! $this->input->hasOption('job-id') || null === $jobId = $this->input->getOption('job-id')) {
            return;
        }
    ...
    private function saveDebugInformation(\Exception $ex = null)
    {
        ...
        $this->getConnection()->executeUpdate(
            "UPDATE system_jobs SET stackTrace = :trace, memoryUsage = :memoryUsage, memoryUsageReal = :memoryUsageReal WHERE id = :id",
            array(
                'id' => $jobId,
    ...
    {
        return $this->getKernel()->getContainer()->get('doctrine')->getManagerForClass('MjrLibraryQueueBundle:Job')->getConnection();
    }
}
