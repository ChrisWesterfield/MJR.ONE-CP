...
namespace Mjr\Library\QueueBundle\Entities\Repository\System;
...
class JobRepository extends EntityRepository
{
    ...
    {
        $dql = <<<DQL
SELECT j FROM LibraryBundle:Job j WHERE j.command = :command AND j.args = :args
DQL;
    ...
    public function getOrCreateIfNotExists($command, array $args = array())
    {
        ...
        $dql = <<<DQL
SELECT j FROM LibraryBundle:Job j WHERE j.command = :command AND j.args = :args ORDER BY j.id ASC
DQL;
        $firstJob = $this->_em->createQuery($dql)
             ->setParameter('command', $command)
    ...
    {
        $sql = <<<SQL
UPDATE system_jobs SET workerName = :worker WHERE id = :id AND workerName IS NULL
SQL;
    ...
    public function findAllForRelatedEntity($relatedEntity)
    {
        ...
        $rsm->addRootEntityFromClassMetadata('LibraryBundle:Job', 'j');

        $sql = "SELECT j.* FROM system_jobs j INNER JOIN system_job_related_entities r ON r.job_id = j.id WHERE r.related_class = :relClass AND r.related_id = :relId";
    ...
    public function findJobForRelatedEntity($command, $relatedEntity, array $states = array())
    {
        ...
        $sql = "SELECT j.* FROM system_jobs j INNER JOIN system_job_related_entities r ON r.job_id = j.id WHERE r.related_class = :relClass AND r.related_id = :relId AND j.command = :command";
    ...
    public function findPendingJob(array $excludedIds = array(), array $excludedQueues = array(), array $restrictedQueues = array())
    {
        ...
        $qb->select('j')->from('MjrLibraryQueueBundle:Job', 'j')
    ...
    public function findIncomingDependencies(Job $job)
    {
        ...
        $dql = <<<DQL
SELECT j, d FROM LibraryBundle:Job j LEFT JOIN j.dependencies d WHERE j.id IN (:ids)
DQL;
        return $this->_em->createQuery($dql)
    ...
    public function getIncomingDependencies(Job $job)
    {
        ...
        $dql = <<<DQL
SELECT j FROM LibraryBundle:Job j WHERE j.id IN (:ids)
DQL;
        return $this->_em->createQuery($dql)
                    ->setParameter('ids', $jobIds)
                    ->getResult();
    ...
    {
        $sql = "SELECT source_job_id FROM system_job_dependencies WHERE dest_job_id = :id";
        $jobIds = $this->_em->getConnection()
            ->executeQuery($sql, array('id' => $job->getId()))
    ...
    {
        $dql = <<<DQL
SELECT j FROM LibraryBundle:Job j WHERE j.state IN (:errorStates) AND j.originalJob IS NULL ORDER BY j.closedAt DESC
DQL;
        return $this->_em->createQuery($dql)
    ...
    {
        $dql = <<<DQL
SELECT DISTINCT j.queue FROM LibraryBundle:Job j WHERE j.state IN (:availableStates)  GROUP BY j.queue
DQL;
        $queues =  $this->_em->createQuery($dql)
    ...
    {
        $dql = <<<DQL
SELECT j.queue FROM LibraryBundle:Job j WHERE j.state IN (:availableStates) AND j.queue = :queue
DQL;
