...
class RedisProfilerStorage implements ProfilerStorageInterface
{
     ...
     * @param int    $ttl
     *
     * @internal param int $lifetime The lifetime to use for the purge
     */
    public function __construct($dsn , $username = '' , $password = '' , $ttl = 86400)
    {
        $this->dsn = $dsn;
    ...
    public function find($ip , $url , $limit , $method , $start = null , $end = null)
    {
        ...
        foreach ($profileList as $item)
        {
            ...
            $statusCode = isset( $values[ 6 ] )
                ? $values[ 6 ]
                : null;

            $itemTime = (int) $itemTime;

            if ($ip && false === strpos($itemIp , $ip) || $url && false === strpos($itemUrl , $url)
                || $method
                   && false === strpos($itemMethod , $method)
            )
            {
                continue;
            }
    ...
    protected function getRedis()
    {
        if (null === $this->redis)
        {
            ...
            if (false === $data || !isset( $data[ 'scheme' ] ) || $data[ 'scheme' ] !== 'redis'
                || !isset( $data[ 'host' ] )
                || !isset( $data[ 'port' ] )
            )
            {
    ...
    private function appendValue($key , $value , $expiration = 0)
    {
        ...
        if ($redis->exists($key))
        {
            ...
            $redis->setTimeout($key , $expiration);
